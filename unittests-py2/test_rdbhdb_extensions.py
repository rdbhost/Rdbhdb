...
class test_Rdbhdb_extensions(unittest.TestCase):
    ...
    def test_DictCursor_fetchone(self):
        con = self._connect()
        try:
            cur = con.cursor(cursor_factory=self.ext.DictCursor)
            ...
            # cursor.fetchone should raise an Error if called after
            # executing a query that cannnot return rows
            cur.execute("INSERT INTO %scities VALUES ('San Francisco');" % self.table_prefix)
            self.assertRaises(self.driver.Error, cur.fetchone)
            ...
            self.assertEqual(r['name'], 'San Francisco',
    ...
    def test_DictCursor_mixedfetch(self):
        ...
        try:
            ...
            rows.sort()
            for i in range(0, len(self.samples)):
                self.assertEqual(rows[i]['name'], self.samples[i],
