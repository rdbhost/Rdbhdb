...
class test_nextset(unittest.TestCase):
    ...
    def test_nextset(self):
        ...
        try:
            ...
            s = cur.nextset()
            self.assert_(s, 'nextset return false neg')
            self.assertRaises(rdbhdb.Error, cur.fetchall)
            self.assert_(cur.rowcount == -1, cur.rowcount)
            self.assert_(not cur._records)
            ...
            self.assert_(len(cur._records)==0)
            
            # no more recordsets
            s = cur.nextset()
            self.assert_(not s, 'nextset return false pos at end of data')
            self.assertRaises(rdbhdb.Error, cur.fetchall)
            self.assert_(cur.rowcount == -1, cur.rowcount)
            self.assert_(not cur._records)
    ...
    def test_limits(self):
        ...
        try:
            ...
            self.assert_(cur.rowcount >= 100-6, cur.rowcount)
            self.assert_(len(r)==100-6, len(r))
