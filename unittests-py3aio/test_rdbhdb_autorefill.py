...
class test_Rdbhdb_autorefill(unittest.TestCase):
    ...
    def test_Auto_Refill_Cursor_fetchone(self):
        ...
        try:
            ...
            self.assertRaises(rdbhdb.Error, cur.fetchone)
            ...
            # cursor.fetchone should raise an Error if called after
            # executing a query that cannnot return rows
            yield from cur.execute("insert into %scities values ('Victoria Bitter')" % (self.table_prefix))
            self.assertRaises(rdbhdb.Error, cur.fetchone)
    ...
    def test_Auto_Refill_Cursor_fetchmany(self):
        ...
        try:
            ...
            #issuing a query
            self.assertRaises(rdbhdb.Error, cur.fetchmany, 4)
    ...
    def test_Auto_Refill_Cursor_refill(self):
        ...
        try:
            cur = con.cursor()
            yield from self.executeDDL3(cur)
            yield from cur.execute('select value from %sTest' % self.table_prefix)
            rows0_94  = []
            ...
            self.assertEqual(rows0_94[90][0], 90, 'fetchmany1 returned incorrect row '+str(rows0_94[90]) )
            self.assertEqual(rows95_104[5][0], 100, 'fetchone returned incorrect row '+str(rows95_104[5]) )
            self.assertEqual(rows105_209[103][0], 208, 'fetchmany2 returned incorrect row '+str(rows105_209[103]) )
            self.assertEqual(rows210_509[101][0], 311, 'fetchall returned incorrect row '+str(rows210_509[101]) )
            self.assertEqual(rows210_509[290][0], 500, 'fetchall returned incorrect row '+str(rows210_509[290]) )
        finally:
            con.close()
