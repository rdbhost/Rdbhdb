...
class test_Rdbhdb_extras(unittest.TestCase):
    ...
    def _logconnect(self):
        try:
            ...
            return self.xtras.AsyncLoggingConnection(host, role, authcode)
        except AttributeError:
            self.fail("No connect method found in self.driver module")
    ...
    def test_Psycopg_Logging_Cursor_fetchone(self):
        ...
        try:
            ...
            self.assertEqual(cur.fetchone(), None,'cursor.fetchone should return None if a query retrieves '
                'no rows')
            self.assertTrue(cur.rowcount in (-1, 0))

            # cursor.fetchone should raise an Error if called after
            # executing a query that cannnot return rows
            yield from cur.execute("insert into %scities values ('Victoria Bitter')" % (
                self.table_prefix))
            self.assertRaises(self.driver.Error, cur.fetchone)
    ...
    def test_Psycopg_Logging_Cursor_fetchmany(self):
        ...
        try:
            ...
            self.assertEqual(len(r), 0,'cursor.fetchmany should return an empty sequence after '
            ...
            self.assertEqual(len(rows), 0, 'cursor.fetchmany should return an empty sequence if '
    ...
    def test_Psycopg_Logging_Cursor_fetchall(self):
        ...
        try:
            ...
            rows = cur.fetchall()
            self.assertTrue(cur.rowcount in (-1, 0))
            self.assertEqual(len(rows), 0,'cursor.fetchall should return an empty list if '
