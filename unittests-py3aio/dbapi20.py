...
class DatabaseAPI20Test(unittest.TestCase):
    ...
    def test_cursor_isolation(self):
        ...
        try:
            ...
            cur1 = con.cursor()
            cur2 = con.cursor()
            yield from self.executeDDL1(cur1)
            yield from cur1.execute("insert into %sbooze values ('Victoria Bitter')" % ( self.table_prefix ))
            yield from cur2.execute("select name from %sbooze" % self.table_prefix)
            booze = cur2.fetchall()
    ...
    def test_description(self):
        ...
        try:
            ...
            yield from cur.execute('select name from %sbooze' % self.table_prefix)
    ...
    def test_rowcount(self):
        ...
        try:
            ...
            yield from cur.execute("insert into %sbooze values ('Victoria Bitter')" % ( self.table_prefix  ))
    ...
    def test_callproc(self):
        ...
        try:
            ...
            if self.lower_func and hasattr(cur, 'callproc'):
                r = yield from cur.callproc(self.lower_func, ('FOO', ))
    ...
    def test_close(self):
        ...
        # closed
        self.assertRaises(self.driver.Error, asyncio_ruc, self.executeDDL1, cur)
    ...
    def test_execute(self):
        ...
        try:
            cur = con.cursor()
            yield from self._paraminsert(cur)
        finally:
            con.close()
    ...
    def _paraminsert(self, cur):
        ...
        if self.driver.paramstyle == 'qmark':
            yield from cur.execute('insert into %sbooze values (?)' % self.table_prefix, ("Cooper's", )  )
        elif self.driver.paramstyle == 'numeric':
            yield from cur.execute('insert into %sbooze values (:1)' % self.table_prefix, ("Cooper's", )  )
        elif self.driver.paramstyle == 'named':
            yield from cur.execute('insert into %sbooze values (:beer)' % self.table_prefix, {'beer':"Cooper's"} )
        elif self.driver.paramstyle == 'format':
            yield from cur.execute('insert into %sbooze values (%%s)' % self.table_prefix, ("Cooper's", ) )
        elif self.driver.paramstyle == 'pyformat':
            yield from cur.execute('insert into %sbooze values (%%(beer)s)' % self.table_prefix, {'beer':"Cooper's"}  )
        else:
    ...
    def test_executemany(self):
        ...
        try:
            ...
            if self.driver.paramstyle == 'qmark':
                yield from cur.executemany('insert into %sbooze values (?)' % self.table_prefix, largs )
            elif self.driver.paramstyle == 'numeric':
                yield from cur.executemany('insert into %sbooze values (:1)' % self.table_prefix, largs )
            elif self.driver.paramstyle == 'named':
                yield from cur.executemany('insert into %sbooze values (:beer)' % self.table_prefix,  margs)
            elif self.driver.paramstyle == 'format':
                yield from cur.executemany('insert into %sbooze values (%%s)' % self.table_prefix, largs )
            elif self.driver.paramstyle == 'pyformat':
                yield from cur.executemany('insert into %sbooze values (%%(beer)s)' % (self.table_prefix), margs )
            else:
    ...
    def test_fetchone(self):
        ...
        try:
            ...
            self.assertTrue(cur.rowcount in (-1, 1), cur.rowcount)
        finally:
            con.close()
    ...
    def test_fetchmany(self):
        ...
        try:
            ...
            yield from cur.execute('select name from %sbooze' % self.table_prefix)
            r = cur.fetchmany()
            self.assertEqual(len(r), 1, 'cursor.fetchmany retrieved incorrect number of rows, '
            ...
            self.assertTrue(cur.rowcount in (-1, 6), cur.rowcount)
            ...
            yield from self.executeDDL2(cur)
            yield from cur.execute('select name from %sbarflys' % self.table_prefix)
            r = cur.fetchmany() # Should get empty sequence
            self.assertEqual(len(r), 0,
                'cursor.fetchmany should return an empty sequence if '
                'query retrieved no rows'
                )
            self.assertTrue(cur.rowcount in (-1, 0), cur.rowcount)
    ...
    def test_fetchall(self):
        ...
        try:
            ...
            yield from cur.execute('select name from %sbooze' % self.table_prefix)
            rows = cur.fetchall()
            self.assertTrue(cur.rowcount in (-1, len(self.samples)), cur.rowcount)
    ...
    def help_nextset_tearDown(self, cur):
        ...
        raise NotImplementedError('Helper not implemented')
        #yield from cur.execute("drop procedure deleteme")
    ...
    def test_None(self):
        ...
        try:
            cur = con.cursor()
            yield from self.executeDDL1(cur)
            yield from cur.execute('insert into %sbooze values (NULL)' % self.table_prefix)
            yield from cur.execute('select name from %sbooze' % self.table_prefix)
            r = cur.fetchall()
            self.assertEqual(len(r), 1)
    ...
    def test_Timestamp(self):
        ...
        # Can we assume this? API doesn't specify, but it seems implied
        # self.assertEqual(str(t1), str(t2))rdbhdb.__version__==need_version
